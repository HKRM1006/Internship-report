...
- **Amazon Rekognition, Textract and Bedrock (Claude)**: the AI layer that labels images, extracts document text, and answers questions or summarizes a document in the same language as the question.
...
4. **Lambda → AI services**: Lambda calls Rekognition for image labels or Textract for document text, and the `ask` endpoint sends stored text to Bedrock (Claude) to answer a question or summarize the document in the language of the question.
...
![InsightShare Architecture](/images/5-Workshop/5.1-Workshop-overview/architecture_5.png)
...
| Amazon Bedrock (Claude) | Answers questions and summarizes a document in the same language as the question | Hosted Claude model; one API call turns the extracted text into an answer |
